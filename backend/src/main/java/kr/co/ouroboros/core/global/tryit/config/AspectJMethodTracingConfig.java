...
public class AspectJMethodTracingConfig implements org.springframework.beans.factory.InitializingBean {
    ...
    public void afterPropertiesSet() {
        ...
        try {
            // Try to get the AspectJ singleton via aspectOf()
            ...
            log.warn("⚠ WARNING: aspectOf() method not found on MethodTracingAspect");
            log.warn("   This indicates AspectJ weaving is not configured in your build.");
            log.warn("   Creating fallback instance - method tracing may not work correctly.");
            log.warn("   Please configure AspectJ weaving as described in docs/ASPECTJ_SETUP.md");
            aspect = new MethodTracingAspect();
        } catch (Exception e) {
            log.error("Failed to retrieve AspectJ singleton instance", e);
            aspect = new MethodTracingAspect();
        }
